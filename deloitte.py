    ...
    if uploaded_file.name.endswith(".pdf"):
        ...
        try:
            ...
            if df.shape[1] >= 2:
                ...
                if not advice:
                    advice.append("Standard tax conditions apply based on available data.")
                    advice.append("No major risks detected — standard deduction paths likely.")
                for line in advice:
                    st.write(f"🧠 {line}")

                st.metric("Total Revenue", f"${revenue_total:,.2f}")
                st.bar_chart(pd.Series(categories).sort_values(ascending=False))
                ...
                suggestions = INDUSTRY_SUGGESTIONS.get(industry, [])
                if suggestions:
                    for item in suggestions:
                        st.write(f"🔹 {item}")
                else:
                     st.info("No specific suggestions for selected industry.")
